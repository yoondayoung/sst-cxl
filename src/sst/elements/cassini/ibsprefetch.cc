...
void IBSPrefetcher::notifyAccess(const CacheListenerNotification& notify)
{
    ...
    const Addr ip = notify.getInstructionPointer();
    ...
    retVal = recentAddrList->insert ( std::pair< uint64_t, StrideFilter >(ip, filterEntry) );
    if( retVal.second == false )
    {
        tempStride = int32_t( addr - (*recentAddrList)[ip].lastAddress );
        if( (*recentAddrList)[ip].state == P_INVALID )
        {
            if( (*recentAddrList)[ip].lastStride == tempStride )
            {
                (*recentAddrList)[ip].state = P_PENDING;
            }
        }
        else if( (*recentAddrList)[ip].state == P_PENDING )
        {
            if( (*recentAddrList)[ip].lastStride == tempStride )
            {
                (*recentAddrList)[ip].state = P_VALID;
                (*recentAddrList)[ip].stride = tempStride;
            }
        ...
        (*recentAddrList)[ip].lastStride = tempStride;
        (*recentAddrList)[ip].lastAddress = addr;
    ...
    if(recheckCountdown == 0)
        DispatchRequest(addr, ip);
}

void IBSPrefetcher::DispatchRequest(Addr targetAddress, Addr ip)
{
    /*  TODO Needs to be updated with current MemHierarchy Commands/States, MemHierarchyInterface */
    MemEvent* ev = NULL;

    int32_t stride = (*recentAddrList)[ip].stride;
...
IBSPrefetcher::IBSPrefetcher(ComponentId_t id, Params& params) : CacheListener(id, params)
{
    ...
    snprintf(new_prefix, sizeof(char)*128, "IBSPrefetcher[%s | @f:@p:@l] ", getName().c_str());
    ...
    output->verbose(CALL_INFO, 1, 0, "IBSPrefetcher created, cache line: %" PRIu64 ", page size: %" PRIu64 "\n",
            blockSize, pageSize);
...
void IBSPrefetcher::registerResponseCallback(Event::HandlerBase* handler)
{
    registeredCallbacks.push_back(handler);
}

void IBSPrefetcher::printStats(Output &out)
{
}
