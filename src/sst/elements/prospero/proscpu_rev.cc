	...
	Component(id)
{
	...
	actmemSize = (uint64_t) params.find<uint64_t>("act_mem_size", 128 * 1024 * 1024 ); // default value act_mem_size = 128Mb
	memSize = (uint64_t) params.find<uint64_t>("mem_size", 1024 * 1024 * 1024); // default value 1GB main memory; 
...
bool ProsperoComponent::tick(SST::Cycle_t currentCycle) {
	...
	//이전 mem trace를 처리했으면 issuedThisCycle != 0 이다, 몇 cycle이 stall 되는지 알 수 있음.
...
void ProsperoComponent::issueRequest(const ProsperoTraceEntry* entry) {
	...
	bool  isRead_ACT            = entry->isRead_ACT();
	bool  isWrite_ACT 			= entry->isWrite_ACT();
	if(isRead) {
